...
function RecentTableBox(props) {
    ...
    return (
        <div className="RecentFiles">
            ...
            <table>
                <tr>
                    <td><div className="check"><input type="checkbox" checked={allchecked} onChange={() => setallchecked(!allchecked)} className='checkbox' />{allchecked && (<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" style={{ cursor: "pointer" }} onClick={() => { props.setTranscriptionRecords([]); if (props.transcriptionRecords.length > 0) props.setNotificationRecords([...props.notificationRecords, "All Records Deleted"]); setallchecked(!allchecked); }}>
                ...
                {props.transcriptionRecords.map((item, index) => (
                    <Tablecell notificationRecords={props.notificationRecords} setNotificationRecords={props.setNotificationRecords} item={item} allchecked={allchecked}></Tablecell>
                ))}
