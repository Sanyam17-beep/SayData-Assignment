                        ...
                        <div className="NotificationIcon"  > {<div className="badge">{props.notificationRecords.length}</div>} <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" onClick={() =>{ if(!showDropdown)setShowDropdown(true); else setShowDropdown(false);}}>
                            ...
                            >
                                ...
                                {props.notificationRecords.length>0&&
                                    (props.notificationRecords.map((element,index)=><NotificationCard>{element}</NotificationCard>))
                                }
                                {props.notificationRecords.length==0&&(<NotificationCard>No Notifications</NotificationCard>)}
                            </Dropdown>)}
                        </div>
                        <div className="ProfileIcon"><img src={props.img} alt="icon" className='image' /></div>
                    </div>
                </div>
            </div>
        </>
